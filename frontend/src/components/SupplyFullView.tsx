...
export const SupplyFullView: React.FC<SupplyFullViewProps> = ({
  ...
  // Навигация с клавиатуры для галереи изображений
  useEffect(() => {
  ...
  }, [open, images.length, currentImageIndex, imageViewerOpen]);
            ...
            <div className="lg:w-1/2 flex flex-col border-b lg:border-b-0 lg:border-r">
              ...
              <div className="flex-1 flex items-center justify-center bg-gray-50 p-2 sm:p-4 relative min-h-[300px] sm:min-h-[400px]">
                <button
                  ...
                  className="absolute inset-0 w-full h-full flex items-center justify-center bg-transparent hover:bg-black/5 transition-colors group cursor-zoom-in"
                >
                  <div className="max-w-full max-h-full flex items-center justify-center">
                    <img
                      src={images[currentImageIndex]?.image}
                      alt={`Изображение ${currentImageIndex + 1}`}
                      className="max-w-full max-h-[60vh] sm:max-h-[70vh] object-contain group-hover:opacity-95 transition-opacity"
                    />
                  </div>
                  <div className="absolute bottom-4 right-4 bg-black/60 text-white px-3 py-2 rounded-lg text-sm opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-2">
                ...
                {images.length > 1 && (
                  <>
                    <Button
                      ...
                      className="absolute left-2 sm:left-4 top-1/2 -translate-y-1/2 h-8 w-8 sm:h-10 sm:w-10 bg-white/80 hover:bg-white z-10"
                    >
                    ...
                    <Button
                      ...
                      className="absolute right-2 sm:right-4 top-1/2 -translate-y-1/2 h-8 w-8 sm:h-10 sm:w-10 bg-white/80 hover:bg-white z-10"
                    >
                ...
                {/* Image Counter */}
                {images.length > 1 && (
                  <div className="absolute top-2 sm:top-4 left-1/2 -translate-x-1/2 bg-black/60 text-white px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm z-10">
                  ...
                  <div className="flex gap-2 h-full">
                    {images.map((image, index) => (
                      <button
                        ...
                        onDoubleClick={() => handleThumbnailClick(index)}
                        className={`relative flex-shrink-0 h-full aspect-square rounded-lg overflow-hidden border-2 transition-all hover:border-blue-300 cursor-pointer ${index === currentImageIndex ? 'border-blue-500' : 'border-transparent'}`}
                      >
                        <img
                          src={image.image}
