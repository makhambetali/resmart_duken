...
class SupplyService:
    ...
    def get_supplies(self, supply_type: str) -> List[SupplyDTO]:
        if supply_type == 'past':
            queryset = self.dao.get_past_supplies()
        else:
            queryset = self.dao.get_future_supplies()
        
        return [self._to_dto(supply) for supply in queryset]

    def get_supplies_by_date(self, target_date: date = None, only_confirmed: bool = True) -> List[SupplyDTO]:
